...
begin
  ...
  AssignFile(txtKey, 'API_Key.txt');
  Rewrite(txtKey);
  Writeln(txtKey, fraSettings1.mmKey.Text);
  CloseFile(txtKey);
end;
...
begin
  ...
  AssignFile(txtKey, 'API_Key.txt');
  Reset(txtKey);
  while not Eof(txtKey) do
    begin
      Readln(txtKey, sKey);
    end;
  CloseFile(txtKey);

  fraSettings1.mmKey.Lines.Clear;
  fraSettings1.mmKey.Lines.Add(sKey);
    ...
    begin
      ...
      with fraAction1 do
        begin
          ...
          // Add Heading
          tblNews.First;
          lblHeading1.Text := 'Pure magic: snow falls on Johannesburg for first time in 11 years';
          ...
          // Add Heading
          tblNews.RecNo := 5;
          lblHeading5.Text := tblNews['Article_Headline'];
          ...
          // Add Author and Source Name
          mmArticle5.Lines.Add(tblNews['Author'] + ' @ ' +
          tblNews['Source_Name']);

          // Add Minor/Major Status and Place
          if tblNews['Major_Event'] = True then
            begin
              mmArticle5.Lines.Add('Major Event in ' + sPlace + #13);
            end
          else
            begin
          ...
          sOriginal := mmArticle5.Lines.Text;
          sAfter1 := (StringReplace(sOriginal, '<div>', '', [rfReplaceAll, rfIgnoreCase]));
          sAfter2 := (StringReplace(sAfter1, '</div>', '', [rfReplaceAll, rfIgnoreCase]));
          mmArticle5.Lines.Clear;
          mmArticle5.Lines.Add(sAfter2);
        end;
    end;
end;
